...
* Check the `ConnectionStrings` in `appsettings.json` files under the `Ecommerce.AuthServer`, `Ecommerce.HttpApi.Host` and `Ecommerce.DbMigrator` projects and change it if you need.
...
* `Ecommerce.AuthServer`: ASP.NET Core MVC / Razor Pages application that is integrated OAuth 2.0(`OpenIddict`) and account modules. It is used to authenticate users and issue tokens.
